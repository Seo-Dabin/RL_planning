...
class AverageDisplacementError(AbstractTrainingMetric):
    ...
    def compute(self, predictions: TargetsType, targets: TargetsType) -> torch.Tensor:
        ...
        wandb.log({"ADE": torch.norm(predicted_trajectory.xy - targets_trajectory.xy, dim=-1).mean()}, commit=True)
...
class FinalDisplacementError(AbstractTrainingMetric):
    ...
    def compute(self, predictions: TargetsType, targets: TargetsType) -> torch.Tensor:
        ...
        wandb.log({"FDE": torch.norm(predicted_trajectory.terminal_position - targets_trajectory.terminal_position, dim=-1).mean()}, commit=True)
...
class AverageHeadingError(AbstractTrainingMetric):
    ...
    def compute(self, predictions: TargetsType, targets: TargetsType) -> torch.Tensor:
        ...
        wandb.log({"AHE": torch.atan2(torch.sin(errors), torch.cos(errors)).mean()}, commit=True)
...
class FinalHeadingError(AbstractTrainingMetric):
    ...
    def compute(self, predictions: TargetsType, targets: TargetsType) -> torch.Tensor:
        ...
        wandb.log({"FHE": torch.atan2(torch.sin(errors), torch.cos(errors)).mean()}, commit=True)
        return torch.atan2(torch.sin(errors), torch.cos(errors)).mean()

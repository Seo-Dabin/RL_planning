...
class mlPolicy:
    ...
    def ml_model(
        ...
        """
        Defines the differential equations for ml.
    ...
    def solve_forward_euler_ml_policy(
        self, agent: mlAgentState, lead_agent: mlLeadAgentState, sampling_time: float
    ) -> mlAgentState:
        """
        ...
        x_dot, v_agent_dot = self.ml_model([], agent.to_array(), lead_agent.to_array(), params)

        # print("ml result : ", [x_dot, v_agent_dot])
        x_dot = 30.0
        v_agent_dot = 0.0
        return mlAgentState(
    ...
    def solve_odeint_ml_policy(
        self, agent: mlAgentState, lead_agent: mlLeadAgentState, sampling_time: float, solve_points: int = 10
    ) -> mlAgentState:
        """
        ...
        return mlAgentState(solution[-1][0], solution[-1][1])

    def solve_ivp_ml_policy(
        self, agent: mlAgentState, lead_agent: mlLeadAgentState, sampling_time: float
    ) -> mlAgentState:
        """
        ...
        # return the last solution
        return mlAgentState(solution.y[0][-1], solution.y[1][-1])

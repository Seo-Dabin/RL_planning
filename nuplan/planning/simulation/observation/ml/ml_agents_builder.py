...
from nuplan.planning.simulation.observation.ml.ml_agent import mlAgent, mlInitialState
from nuplan.planning.simulation.observation.ml.ml_agent_manager import UniquemlAgents
...
def build_ml_agents_on_map_rails(
...
) -> Tuple[UniquemlAgents, OccupancyMap]:
    """
    ...
    :return: a dictionary of ml agent uniquely identified by a track_token
    """
    unique_agents: UniquemlAgents = {}
    ...
    for agent in tqdm(
        ...
        if agent.track_token not in unique_agents:
            ...
            # Add to occupancy_map to pass on to mlAgentManger
            ...
            unique_agents[agent.track_token] = mlAgent(
                start_iteration=0,
                initial_state=initial_state,
                route=[route],
                policy=mlPolicy(target_velocity, min_gap_to_lead_agent, headway_time, accel_max, decel_max),

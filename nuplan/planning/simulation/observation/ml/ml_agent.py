...
from nuplan.planning.simulation.observation.ml.ml_policy import mlPolicy
from nuplan.planning.simulation.observation.ml.ml_states import mlAgentState, mlLeadAgentState
from nuplan.planning.simulation.observation.ml.utils import create_path_from_se2, path_to_linestring
...
class mlInitialState:
    """Initial state of mlAgent."""
...
class mlAgent:
    ...
    def __init__(
        ...
        self._state = mlAgentState(initial_state.path_progress, initial_state.velocity.x)
    ...
    def propagate(self, lead_agent: mlLeadAgentState, tspan: float) -> None:
        """
        Propagate agent forward according to the ml policy.
        ...
        solution = self._policy.solve_forward_euler_ml_policy(
            mlAgentState(0, self._state.velocity), lead_agent, tspan
        )
    ...
    def get_route(self) -> List[LaneGraphEdgeMapObject]:
        """:return: The route the ml agent is following."""
